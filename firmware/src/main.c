...
void but_callback_on_off(void) {
    if (first_time == 1) {
        first_time = 0;
        char on_off = '1';
        xQueueSendFromISR(xQueueOnOff, &on_off, 0);
    } else {
        first_time = 1;
        char on_off = '0';
        xQueueSendFromISR(xQueueOnOff, &on_off, 0);
...
void on_off_init(void) {
    pmc_enable_periph_clk(BUT_PIO_ID_ON_OFF);
    pio_set_debounce_filter(BUT_PIO_ON_OFF, BUT_IDX_MASK_ON_OFF, 120);
    pio_configure(BUT_PIO_ON_OFF, PIO_INPUT, BUT_IDX_MASK_ON_OFF, PIO_PULLUP | PIO_DEBOUNCE);
    pio_handler_set(BUT_PIO_ON_OFF, BUT_PIO_ID_ON_OFF, BUT_IDX_MASK_ON_OFF, PIO_IT_FALL_EDGE, but_callback_on_off);
    pio_enable_interrupt(BUT_PIO_ON_OFF, BUT_IDX_MASK_ON_OFF);
    pio_get_interrupt_status(BUT_PIO_ON_OFF);
    NVIC_EnableIRQ(BUT_PIO_ID_ON_OFF);
    NVIC_SetPriority(BUT_PIO_ID_ON_OFF, 4);
}
...
int main(void) {
    ...
    // Queue on off
    xQueueOnOff = xQueueCreate(1, sizeof(char));
    if (xQueueOnOff == NULL) {
        printf("Erro ao criar fila de on off \n");
    }
